...
router.post('/pay', authenticate, authorize('customer'),
  ...
  body('card_expiry').notEmpty().trim(),
  ...
  async (req, res, next) => {
    ...
    const { order_id, card_number } = req.body;
    ...
    try {
      ...
      try {
        ...
        if (!existingPayment.length) {
          const [result] = await conn.query(
            `INSERT INTO payments
             ...
             VALUES (?,?,?,?,?,?,?,?)`,
            [order_id, rows[0].customer_id, rows[0].price, rows[0].currency || 'EUR', status, 'card', fakeRef, isValid ? new Date() : null]
          );
        ...
        if (isValid) {
          return res.json({ success: true, payment_id: paymentId, reference: fakeRef, message: 'Pago realizado correctamente. Tu pedido está en proceso.' });
        } else {
          return res.status(402).json({ success: false, payment_id: paymentId, message: 'Tarjeta rechazada. Comprueba los datos e inténtalo de nuevo.' });
        }
      } catch (e) {
        await conn.rollback();
        conn.release();
